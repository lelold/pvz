...
func (_m *PVZRepo) GetReceptionsWithProducts(pvzID uuid.UUID) ([]model.ReceptionWithProducts, error) {
	ret := _m.Called(pvzID)
	...
	var r1 error
	if rf, ok := ret.Get(0).(func(uuid.UUID) ([]model.ReceptionWithProducts, error)); ok {
		return rf(pvzID)
	}
	if rf, ok := ret.Get(0).(func(uuid.UUID) []model.ReceptionWithProducts); ok {
		r0 = rf(pvzID)
	} else {
		if ret.Get(0) != nil {
